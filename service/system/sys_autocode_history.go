...
import (
	"errors"
	"fmt"
	systemReq "go-generate/model/system/request"
	"path/filepath"
	"strconv"
	"strings"
	"time"

	"go-generate/model/system/response"
...
func (autoCodeHistoryService *AutoCodeHistoryService) CreateAutoCodeHistory(meta, structName, structCNName, autoCodePath string, injectionMeta string, tableName string, apiIds string, Package string) error {
	return global.YAN_DB.Create(&system.SysAutoCodeHistory{
		Package:       Package,
		RequestMeta:   meta,
...
func (autoCodeHistoryService *AutoCodeHistoryService) First(info *request.GetById) (string, error) {
	var meta string
	return meta, global.YAN_DB.Model(system.SysAutoCodeHistory{}).Select("request_meta").Where("id = ?", info.Uint()).First(&meta).Error
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) Repeat(businessDB, structName, Package string) bool {
	var count int64
	global.YAN_DB.Model(&system.SysAutoCodeHistory{}).Where("business_db = ? and struct_name = ? and package = ? and flag = 0", businessDB, structName, Package).Count(&count)
	return count > 0
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) RollBack(info *systemReq.RollBack) error {
	...
	if err := global.YAN_DB.Where("id = ?", info.ID).First(&md).Error; err != nil {
		return err
	}
	// 清除API表
	...
	if err != nil {
		global.YAN_LOG.Error("ClearTag DeleteApiByIds:", zap.Error(err))
	}
	// 删除表
	if info.DeleteTable {
		if err = AutoCodeServiceApp.DropTable(md.BusinessDB, md.TableName); err != nil {
			global.YAN_LOG.Error("ClearTag DropTable:", zap.Error(err))
		}
	}
	// 删除文件
	...
	for _, path := range strings.Split(md.AutoCodePath, ";") {
		...
		// 迁移
		nPath := filepath.Join(global.YAN_CONFIG.AutoCode.Root,
		...
		if err != nil {
			global.YAN_LOG.Error("file move err ", zap.Error(err))
		}
	...
	md.Flag = 1
	return global.YAN_DB.Save(&md).Error
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) Delete(info *request.GetById) error {
	return global.YAN_DB.Where("id = ?", info.Uint()).Delete(&system.SysAutoCodeHistory{}).Error
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) GetList(info request.PageInfo) (list []response.AutoCodeHistory, total int64, err error) {
	...
	db := global.YAN_DB.Model(&system.SysAutoCodeHistory{})
