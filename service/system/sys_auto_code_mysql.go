...
func (s *autoCodeMysql) GetDB(businessDB string) (data []response.Db, err error) {
	...
	if businessDB == "" {
		err = global.YAN_DB.Raw(sql).Scan(&entities).Error
	} else {
		err = global.YAN_DBList[businessDB].Raw(sql).Scan(&entities).Error
	}
	return entities, err
}
...
func (s *autoCodeMysql) GetTables(businessDB string, dbName string) (data []response.Table, err error) {
	...
	if businessDB == "" {
		err = global.YAN_DB.Raw(sql, dbName).Scan(&entities).Error
	} else {
		err = global.YAN_DBList[businessDB].Raw(sql, dbName).Scan(&entities).Error
	}
...
func (s *autoCodeMysql) GetColumn(businessDB string, tableName string, dbName string) (data []response.Column, err error) {
	...
	`
	if businessDB == "" {
		err = global.YAN_DB.Raw(sql, tableName, dbName).Scan(&entities).Error
	} else {
		err = global.YAN_DBList[businessDB].Raw(sql, tableName, dbName).Scan(&entities).Error
	}
